...
//                          + Global scrape activity banner (everyone)
...
// Keep in sync with COMPLETED_LOCK_HOURS in api_server.js
const COMPLETED_LOCK_HOURS = 48;
...
// ── Poll global scraper activity — any user, any category ─────
function useActiveScraperJobs(pollMs = 6000) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const jobs = await getActiveScraperJobs();
        if (!cancelled) setActiveJobs(jobs);
      } catch (err) {
        console.warn('Active scraper jobs poll failed:', err.message);
      }
    }
...
// ─────────────────────────────────────────────────────────────
// Global Scrape Activity Banner — visible to EVERYONE on Settings,
// not just admin/supervisor. Replaces the static description text
// with live cross-user activity whenever something's running or was
// recently completed/failed.
// ─────────────────────────────────────────────────────────────
function GlobalScrapeActivityBanner() {
  const activeJobs = useActiveScraperJobs();

  function statusLine(job) {
    const who  = job.startedBy || 'Someone';
    const cats = job.categoryNames?.length ? job.categoryNames.join(', ') : '—';
    const when = fmtDate(job.startedAt);

    if (job.status === 'running') {
      return { icon: '🟢', color: 'text-emerald-400',
        text: `${who} started scraping ${cats} at ${when} — still running` };
    }
    if (job.status === 'done') {
      return { icon: '✅', color: 'text-slate-300',
        text: `${who} scraped ${cats} at ${when} — completed (locked for ${COMPLETED_LOCK_HOURS}h)` };
    }
    if (job.status === 'cancelled') {
      return { icon: '🛑', color: 'text-orange-400',
        text: `${who} cancelled a scrape of ${cats} at ${when} — free to retry` };
    }
    return { icon: '❌', color: 'text-red-400',
      text: `${who}'s scrape of ${cats} at ${when} failed — free to retry` };
  }

  return (
    <div className="px-4 py-3 rounded-xl border border-slate-700/60 bg-slate-800/60 mb-3">
      {activeJobs.length === 0 ? (
        <p className="text-sm font-semibold text-white">
          Scrapes all due categories now, regardless of scheduled time. Only scrapes categories configured.
        </p>
      ) : (
        <div className="space-y-1.5">
          ...
          {activeJobs.map(job => {
            ...
            return (
              <p key={job.jobId} className={`text-sm ${line.color}`}>
                {line.icon} {line.text}
              </p>
            );
          })}
...
function ScraperControl({ user, settingsData = [] }) {
  ...
  // ── Global scraper activity — used to block/grey-out categories
  // that another user is currently scraping or completed recently ──
  const activeJobs = useActiveScraperJobs();

  const blockedCategoryMap = useMemo(() => {
    const map = new Map();
    const now = Date.now();
    for (const job of activeJobs) {
      if (!job.categoryNames) continue;
      let blocks = job.status === 'running';
      if (!blocks && job.status === 'done' && job.finishedAt) {
        const hoursSince = (now - new Date(job.finishedAt).getTime()) / 3_600_000;
        blocks = hoursSince < COMPLETED_LOCK_HOURS;
      }
      if (!blocks) continue;
      for (const cat of job.categoryNames) {
        if (!map.has(cat)) map.set(cat, job);
      }
    }
    return map;
  }, [activeJobs]);
  ...
  function selectAllFilteredCategories() {
    setSelectedCategories(prev => {
      ...
      filteredCategories.forEach(category => {
        if (!blockedCategoryMap.has(category.categoryName)) next.add(category.categoryName);
      });
      return next;
    });
  }
    ...
    try {
      ...
      if (data?.conflicts?.length) {
        const detail = data.conflicts
          .map(c => `${c.categoryName} (${c.status === 'running' ? `being scraped by ${c.startedBy}` : `done by ${c.startedBy}`})`)
          .join(', ');
        setMsg(`Blocked: ${detail}`);
      } else {
        setMsg((data?.error || err.message || 'Failed to start').substring(0, 160));
      }
      setTimeout(() => { setScraperStatus('idle'); setMsg(''); setElapsed(0); }, 10000);
    }
  }
  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex items-center justify-between px-4 py-3
        ...
        <div>
          <p className="text-sm font-semibold text-white">
            Select categories below, then run.
          </p>
        </div>
      ...
      <div className="mb-3 rounded-xl border border-slate-700/60 bg-slate-800/60 p-4">
        ...
        <div className="max-h-56 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-2">
          {filteredCategories.map(cat => {
            ...
            const blocker = blockedCategoryMap.get(cat.categoryName);
              ...
              >
                ...
                {blocker && (
                  <span className="text-[10px] text-amber-400 mr-2">
                    🔒 {blocker.status === 'running' ? `${blocker.startedBy} is scraping` : `done by ${blocker.startedBy}`}
                  </span>
                )}
                {lastScraped && !blocker && (
...
function BusinessRow({ row, onSaved }) {
  ...
  function handleEdit() {
    setFields({
      ...
      PPLookbackDays: row.PPLookbackDays ?? '',
    });
    setEditing(true);
  ...
  function validateField(name, val) {
    ...
    if (name === 'PPLookbackDays') {
      if (isNaN(n) || n < 1 || n > 365) return 'PP Lookback must be 1–365 days';
      return null;
    }
  ...
  async function handleSave() {
    ...
    try {
      const payload = {
        ...
        PPLookbackDays: fields.PPLookbackDays === '' ? null : parseInt(fields.PPLookbackDays, 10),
  ...
  // Multiplier is still computed (used for the tooltip below and by the
  // recommendation engine server-side) — just no longer shown as its own
  // table column per product decision; PP Lookback takes that slot instead.
  ...
  return (
    <tr className="border-b border-slate-800 hover:bg-slate-800/30 transition-colors">
      <td className="px-4 py-3 text-left">
        <span className="font-medium text-slate-200 text-sm" title={`Multiplier: ×${multiplier}`}>
          {row.CategoryName}
        </span>
      </td>
      ...
      <td className="px-4 py-3 text-center">
        {editing ? (
          <DaysInput value={fields.PPLookbackDays} onChange={v => setFields(f => ({ ...f, PPLookbackDays: v }))} placeholder={`${SYSTEM_DEFAULTS.PPLookbackDays} (default)`} />
        ) : (
          <ValueOrDefault value={row.PPLookbackDays} defaultVal={SYSTEM_DEFAULTS.PPLookbackDays} suffix=" days" decimals={0} />
        )}
      </td>
      <td className="px-4 py-3 text-center">
        {row.UpdatedBy ? (
...
// Same shape as PercentInput but for whole-day windows (1–365, integer steps).
function DaysInput({ value, onChange, placeholder }) {
  return (
    <div className="flex items-center justify-center gap-0.5">
      <input type="number" min="1" max="365" step="1" value={value}
        onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="w-20 px-2 py-1 text-xs text-slate-100 bg-slate-700 border border-slate-600
          rounded-lg outline-none text-center focus:border-violet-500 placeholder:text-slate-600
          [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
...
function ValueOrDefault({ value, defaultVal, suffix = '%', decimals = 1 }) {
  if (value != null) {
    return (
      <span className="text-slate-200 text-sm font-medium">
  ...
  return (
    <span className="text-slate-500 text-xs italic">
      {defaultVal}{suffix} <span className="text-slate-600">(default)</span>
    </span>
  );
}
    ...
    <div className="min-h-screen bg-[#0f1117] font-sans">
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-8">
        ...
        {!loading && !error && activeTab === TABS.BUSINESS && (
          <>
            ...
            <div className="flex items-center gap-6 px-4 py-2.5 rounded-xl bg-violet-900/20
              border border-violet-700/40 mb-5 text-xs flex-wrap">
              ...
              <span className="text-violet-300">PP Lookback: {SYSTEM_DEFAULTS.PPLookbackDays} days</span>
            </div>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-800/80 border-b border-slate-700">
                    {['Category', 'GST (%)', 'Cost of Business (%)', 'Profit Margin (%)', 'PP Lookback (days)', 'Last Updated By', 'Action'].map(h => (
        ...
        {!loading && !error && activeTab === TABS.SCRAPING && (
          <>
            {/* Global scraping activity — visible to everyone */}
            <GlobalScrapeActivityBanner />
...
// // ─────────────────────────────────────────────────────────────
// // Bill Lookback control — editable global setting used by
// // fetchPurchasePrices() (azureSqlService.js) to decide how far back to
// // look for a Zoho bill when computing PP. A SKU with no bill inside this
// // window gets PP = null on the next sync.
// // Stored in AppSettings as a plain integer (e.g. "30"), same table/pattern
// // as VARIANCE_THRESHOLD — just no percent conversion needed.
// // ─────────────────────────────────────────────────────────────
// function BillLookbackControl() {
//   const [input, setInput]   = useState('');
//   const [status, setStatus] = useState('idle'); // idle | loading | saving | saved | error
//   const [meta, setMeta]     = useState(null);

//   useEffect(() => {
//     fetchAppSetting('BILL_LOOKBACK_DAYS')
//       .then(data => {
//         const num = parseInt(data.SettingValue, 10);
//         setInput(isNaN(num) ? '' : String(num));
//         setMeta(data);
//         setStatus('idle');
//       })
//       .catch(() => setStatus('error'));
...
//     setStatus('saving');
//     try {
//       const data = await updateAppSetting('BILL_LOOKBACK_DAYS', String(days));
//       setMeta(data);
//       setStatus('saved');
//       setTimeout(() => setStatus('idle'), 2000);
//     } catch {
...
//   return (
//     <div className="flex items-center gap-4 px-4 py-2.5 rounded-xl bg-sky-900/20
//       border border-sky-700/40 mb-5 text-xs flex-wrap">
//       <span className="text-slate-500 font-medium">PP Bill Lookback Window</span>
//       <div className="flex items-center gap-1.5">
//         <input
//           type="number"
//           value={input}
//           onChange={e => setInput(e.target.value)}
//           className="w-16 px-2 py-1 text-xs text-slate-100 bg-slate-700 border border-slate-600
//             rounded-lg outline-none text-center focus:border-sky-500
//             [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
//         />
...
//         onClick={handleSave}
//         disabled={status === 'saving'}
//         className="px-3 py-1 text-[11px] font-medium rounded-lg bg-sky-600 hover:bg-sky-500
//           disabled:opacity-50 text-white transition-colors">
//         {status === 'saving' ? 'Saving…' : 'Save'}
//       </button>
//       {status === 'saved' && <span className="text-emerald-400">✓ Saved</span>}
